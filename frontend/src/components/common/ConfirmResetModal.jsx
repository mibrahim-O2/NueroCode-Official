...
export default function ConfirmResetModal({ title, items, onCancel, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onCancel}>
      <div
        className="flex w-full max-w-md flex-col gap-4 rounded-dialog border border-border bg-charcoal p-6 shadow-dialog"
      ...
      >
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            ...
            disabled={busy}
            className="flex items-center gap-2 rounded-button bg-status-error px-4 py-2 text-sm text-white transition-colors duration-200 hover:bg-status-error/80 disabled:opacity-50"
          >
